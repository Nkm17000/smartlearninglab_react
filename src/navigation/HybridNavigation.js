  ...
  return <View style={{flex:1,backgroundColor:colors.background}}>
    <View style={{backgroundColor:colors.surface,borderBottomWidth:1,borderBottomColor:colors.border,zIndex:10}}>
      <View style={{maxWidth:1440,width:'100%',alignSelf:'center',paddingHorizontal:20,paddingVertical:10,flexDirection:'row',alignItems:'center',gap:10}}>
        ...
        {!compact && <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{gap:5,alignItems:'center',flexGrow:1}}>
    ...
    <View style={{flex:1,flexDirection:'row',maxWidth:1440,width:'100%',alignSelf:'center'}}>
      {!compact && <View style={{width:220,borderRightWidth:1,borderRightColor:colors.border,backgroundColor:colors.surface,padding:12}}>
    ...
    {compact && drawer && <View style={{position:'absolute',top:0,left:0,right:0,bottom:0,zIndex:50,flexDirection:'row'}}>
      ...
      <View style={{position:'absolute',left:0,top:0,bottom:0,width:310,backgroundColor:colors.surface,padding:18,shadowColor:'#000',shadowOpacity:0.15,shadowRadius:20}}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <Text style={{fontFamily:colors.fontFamily,fontSize:9,fontWeight:'900',color:colors.subtle,letterSpacing:1.2,marginBottom:8}}>MAIN MENU</Text>
          {top.map(([r,icon,label])=><Pressable key={r} onPress={()=>go(r)} style={{padding:13,borderRadius:12,marginBottom:6,backgroundColor:active===r?colors.blueSoft:'#fff'}}>
            <View style={{flexDirection:'row',alignItems:'center',gap:9}}><Text style={{fontSize:16,width:22,textAlign:'center'}}>{icon}</Text><Text style={{fontFamily:colors.fontFamily,fontWeight:'900',color:active===r?colors.primary:colors.navy}}>{label}</Text></View>
          </Pressable>)}
          <View style={{height:1,backgroundColor:colors.border,marginVertical:10}}/>
          <Text style={{fontFamily:colors.fontFamily,fontSize:9,fontWeight:'900',color:colors.subtle,letterSpacing:1.2,marginBottom:8}}>LEARNING MENU</Text>
