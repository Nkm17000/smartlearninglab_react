...
export default function AdminResourceManager({scope,id,compact=false,onClose,managerTitle}){
 ...
 if(compact){return <Card style={{backgroundColor:'#F8F9FD',marginTop:8}}><View style={{flexDirection:'row',alignItems:'center',gap:10}}><View style={{flex:1}}><Text style={{fontWeight:'900',color:colors.navy}}>{heading}</Text><Text style={{fontSize:12,color:colors.muted,marginTop:3}}>{items.length?`${items.length} resource${items.length===1?'':'s'} added.`:'No resources added.'}</Text></View><Badge tone="purple">{items.length}</Badge></View>{list}</Card>}
