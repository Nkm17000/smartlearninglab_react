...
export default function FlashcardsScreen(){
 ...
 useEffect(()=>{load()},[]);
 const add=async()=>{try{await api.createFlashcard({front,back});setFront('');setBack('');setShowAdd(false);setRevealed(null);load()}catch(e){console.warn('[Student API] Flashcard:', e?.message || e)}};
 const review=async(id,q)=>{try{await api.reviewFlashcard(id,{quality:q});setRevealed(null);load()}catch(e){console.warn('[Student API] Flashcard review:', e?.message || e)}};
 const remove=async(card)=>{const id=api.idOf(card);const message=`Remove "${card.front||'this card'}" from your deck?`;const doDelete=async()=>{try{setDeleting(id);await api.deleteFlashcard(id);setRevealed(null);await load()}catch(e){console.warn('[Student API] Delete flashcard:', e?.message || e)}finally{setDeleting('')}};if(Platform.OS==='web'&&typeof window!=='undefined'){if(window.confirm(`Delete flashcard?\n\n${message}`))await doDelete();return;}Alert.alert('Delete flashcard?',message,[{text:'Cancel',style:'cancel'},{text:'Delete',style:'destructive',onPress:doDelete}]);};
