...
export default function StudentLibraryScreen(){
 ...
 {items.length===0?<Empty title="No resources in this category" message="Try another category or check back later."/>:<View style={{flexDirection:'row',flexWrap:'wrap',gap:14}}>{items.map(item=><Card key={api.idOf(item)} style={{width:310,minWidth:260}}><Text style={{fontSize:30}}>{icons[item.type]||'📎'}</Text><Text style={{fontSize:17,fontWeight:'900',color:colors.navy,marginTop:8}} numberOfLines={2}>{item.title}</Text><View style={{flexDirection:'row',gap:7,marginTop:8}}><Badge>{item.category||'General'}</Badge><Badge tone="purple">{item.type||'resource'}</Badge></View>{item.description&&<Text style={{fontSize:12,color:colors.muted,lineHeight:18,marginTop:8}}>{item.description}</Text>}<View style={{flexDirection:'row',gap:8,flexWrap:'wrap',marginTop:12}}>
