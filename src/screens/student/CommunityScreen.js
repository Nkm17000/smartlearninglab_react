...
import {AppShell,Button,Card,Field,Header,Loading,Empty} from '../../components/UI';
import {api} from '../../services/api';
import {colors} from '../../theme';
export default function CommunityScreen(){
 const [posts,setPosts]=useState(null),[show,setShow]=useState(false),[title,setTitle]=useState(''),[content,setContent]=useState('');
 const load=()=>api.communityPosts().then(x=>setPosts(api.listOf(x))).catch(e=>Alert.alert('Community',e.message));useEffect(load,[]);
