...
export default function StudentQuizScreen({quizId,onBack,backLabel="Back to Quizzes"}) {
 ...
 return <AppShell>
   ...
   <View style={{flexDirection:mobile?'column':'row',gap:14,alignItems:'stretch'}}>
     <View style={{flex:1}}>
       <Card style={{backgroundColor:colors.hero,borderColor:colors.hero,padding:20}}><View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'flex-start',gap:10}}><View style={{flex:1}}><Text style={{fontFamily:colors.fontFamily,fontSize:10,fontWeight:'900',color:'#AFA8FF',letterSpacing:1.1}}>TEST SERIES • QUIZ</Text><Text style={{fontFamily:colors.fontFamily,fontSize:24,fontWeight:'900',color:'#fff',marginTop:5}}>{quiz.title||quiz.name}</Text><Text style={{fontFamily:colors.fontFamily,color:'#D6D8E2',fontSize:11,marginTop:5}}>{quiz.subject||'General'} · {questions.length} Questions · Pass {quiz.passing_percentage||60}%</Text><View style={{flexDirection:'row',gap:6,flexWrap:'wrap',marginTop:9}}>{(Array.isArray(quiz.categories)?quiz.categories:(quiz.category?[quiz.category]:['General'])).map(x=><Badge key={x} tone="purple">{x}</Badge>)}{attemptMeta?.is_completed&&<Badge tone="green">Completed in another exam category</Badge>}</View></View>{attempt&&<Button title={busy?'Submitting…':'Submit Quiz'} onPress={submit} disabled={busy}/>}</View></Card>
