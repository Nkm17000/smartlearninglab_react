...
export default function StudentHomeScreen({user,openCourse,openQuiz,openRoute}){
 ...
 const [search,setSearch]=useState(''),[activeCategory,setActiveCategory]=useState(''),[activeExam,setActiveExam]=useState(''),[data,setData]=useState(null),[error,setError]=useState('');
 const load=async()=>{try{setError('');setData(await api.home())}catch(e){setError(e?.message||'Unable to load your learning home.')}};
 useEffect(()=>{load()},[]);
 const searchCourses=async(category=activeCategory,exam=activeExam)=>{try{const query=[search,exam].filter(Boolean).join(' ');const c=await api.studentCourses({search:query,category,level:'',language:''});setData(x=>({...x,searchResults:api.listOf(c)}))}catch(e){Alert.alert('Search',e.message)}};
 ...
 const d=data.dashboard||{}; const goal=d.weekly_goal||{target:5,completed:0,percentage:0}; const cont=d.continue_learning; const courses=data.searchResults||data.courses||[]; const categories=data.catalog?.categories?.length?data.catalog.categories:fallbackCats; const catalogExams=data.catalog?.exams||[];
 ...
 return <AppShell>
   ...
   <View style={{flexDirection:'row',flexWrap:'wrap',gap:8}}>{[...catalogExams,...exams.filter(x=>!catalogExams.includes(x))].slice(0,10).map(x=><Pressable key={x} onPress={()=>{const next=activeExam===x?'':x;setActiveExam(next);setActiveCategory('');searchCourses('',next)}} style={{paddingHorizontal:14,paddingVertical:10,borderRadius:13,borderWidth:1,borderColor:activeExam===x?colors.primary:colors.border,backgroundColor:activeExam===x?colors.primary:'#fff'}}><Text style={{fontFamily:colors.fontFamily,fontWeight:'900',fontSize:11,color:activeExam===x?'#fff':colors.text}}>{x}</Text></Pressable>)}</View>

   <SectionTitle title={data.searchResults?'Search Results':'Featured Courses'} subtitle="Curated learning paths with practice built in." right={<Text style={{fontFamily:colors.fontFamily,fontSize:11,fontWeight:'900',color:colors.primary}}>View all →</Text>}/>
   {courses.length===0?<Empty title="No courses found" message="Try another search or exam category."/>:<View style={{flexDirection:'row',flexWrap:'wrap',gap:12}}>{courses.slice(0,10).map(c=><CourseCard key={api.idOf(c)} c={c} onOpen={openCourse}/>)}</View>}

   <SectionTitle title="Test Series" subtitle="Practice with timed quizzes and mock tests." />
   {data.quizzes?.length?<View style={{flexDirection:'row',flexWrap:'wrap',gap:12}}>{data.quizzes.slice(0,6).map(q=><QuizCard key={api.idOf(q)} q={q} onOpen={openQuiz}/>)}</View>:<Empty title="No published tests yet" message="Published quizzes will appear here."/>}
