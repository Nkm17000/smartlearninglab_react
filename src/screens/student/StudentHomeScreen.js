...
function Stat({icon,title,value,delta,tone='blue',onPress}){
 const bg=tone==='green'?colors.greenSoft:tone==='orange'?colors.orangeSoft:tone==='pink'?colors.pinkSoft:colors.blueSoft;
 return <Pressable onPress={onPress} disabled={!onPress} style={({pressed})=>({flex:1,minWidth:0,opacity:pressed?0.9:1})}>
   <Card style={{marginBottom:0,padding:14,height:'100%'}}>
     <View style={{flexDirection:'row',alignItems:'center',gap:9}}>
       <View style={{width:36,height:36,borderRadius:11,backgroundColor:bg,alignItems:'center',justifyContent:'center'}}><Text style={{fontSize:17}}>{icon}</Text></View>
       <Text style={{fontFamily:colors.fontFamily,fontSize:11,fontWeight:'800',color:colors.muted,flex:1}} numberOfLines={2}>{title}</Text>
     </View>
...
function CourseCard({c,onOpen}){
 ...
 return <Pressable onPress={()=>onOpen(api.idOf(c))} style={({pressed})=>({flex:1,minWidth:0,width:'100%',opacity:pressed?.84:1})}>
...
function QuizCard({q,onOpen}){return <Card style={{flex:1,minWidth:0,marginBottom:0}}><View style={{flexDirection:'row',gap:10,alignItems:'center'}}><View style={{width:42,height:42,borderRadius:13,backgroundColor:colors.purpleSoft,alignItems:'center',justifyContent:'center'}}><Text style={{fontSize:18}}>📝</Text></View><View style={{flex:1}}><Text style={{fontFamily:colors.fontFamily,fontWeight:'900',color:colors.navy}} numberOfLines={2}>{q.title||q.name}</Text><Text style={{fontFamily:colors.fontFamily,fontSize:10,color:colors.muted,marginTop:3}}>{q.duration_minutes||15} min · {(q.question_ids||[]).length} questions</Text></View></View><Button title="Take Test" onPress={()=>onOpen(api.idOf(q))} style={{marginTop:12}}/></Card>}

function Hero({search,setSearch,onSearch,mobile}){
 return <View style={{borderRadius:20,overflow:'hidden',backgroundColor:colors.hero,padding:22,position:'relative'}}>
   ...
   <View style={{maxWidth:760}}>
     ...
     <View style={{flexDirection:mobile?'column':'row',gap:8,marginTop:18,alignItems:'stretch'}}><View style={{flex:1,minWidth:0}}><Field value={search} onChangeText={setSearch} placeholder="Search courses, exams, topics…"/></View><Button title="Search" onPress={onSearch} style={{width:mobile?'100%':undefined}}/></View>
   </View>
 </View>;
}
...
export default function StudentHomeScreen({user,openCourse,openQuiz,openRoute}){
 ...
 useEffect(()=>{load()},[]);
 const searchCourses=async(category=activeCategory,exam=activeExam)=>{try{const query=[search,exam].filter(Boolean).join(' ');const c=await api.studentCourses({search:query,category,exam,level:'',language:''});setData(x=>({...x,searchResults:api.listOf(c)}))}catch(e){setError(e?.message||'Search could not be completed.')}};
 ...
 const stats=[
   ['📖','Courses',d.courses_available||0,'+1 this week','blue','courses'],
   ['✓','Lessons Done',d.lessons_completed||0,'+5 this week','blue','progress'],
   ['🎯','Quiz Average',`${d.quiz_average||0}%`,d.quiz_average?'+4.2%':'Start practicing','pink','quizzes'],
   ['▤','Tests Attempted',d.quiz_attempts||0,'+2 this week','blue','quizzes'],
   ['🔥','Current Streak',`${d.streak?.current||0} days`,d.streak?.current?'Keep it up!':'Start today','orange','gamification'],
   ['★','XP Points',d.xp||0,'+120 this week','pink','gamification']
 ];
 const statRows=mobile ? stats.slice(0,4) : stats;
 return <AppShell>
   <View style={{flexDirection:wide?'row':'column',gap:14,alignItems:'stretch'}}>
     <View style={{flex:1,minWidth:0}}>
       <Hero search={search} setSearch={setSearch} onSearch={()=>searchCourses(activeCategory,activeExam)} mobile={mobile}/>
       <View style={{flexDirection:'row',flexWrap:'wrap',gap:10,marginTop:14}}>{statRows.map(([i,t,v,delta,tone,r])=><View key={t} style={{width:mobile?'48.2%':undefined,flex:mobile?undefined:1}}><Stat icon={i} title={t} value={v} delta={delta} tone={tone} onPress={()=>openRoute&&openRoute(r)}/></View>)}</View>
     </View>
   ...
   <SectionTitle title={data.searchResults?'Search Results':'Featured Courses'} subtitle="Top 5 courses on your dashboard. Open Courses to see everything." right={<Pressable onPress={()=>openRoute&&openRoute('courses')}><Text style={{fontFamily:colors.fontFamily,fontSize:11,fontWeight:'900',color:colors.primary}}>View all →</Text></Pressable>}/>
   {courses.length===0?<Empty title="No courses found" message="Try another search or exam category."/>:<View style={{flexDirection:mobile?'column':'row',flexWrap:mobile?'nowrap':'wrap',gap:12}}>{courses.slice(0,5).map(c=><CourseCard key={api.idOf(c)} c={c} onOpen={openCourse}/>)}</View>}

   <SectionTitle title="Test Series" subtitle="Top 5 quizzes on your dashboard. Open Quizzes to see everything." right={<Pressable onPress={()=>openRoute&&openRoute('quizzes')}><Text style={{fontFamily:colors.fontFamily,fontSize:11,fontWeight:'900',color:colors.primary}}>View all →</Text></Pressable>}/>
   {data.quizzes?.length?<View style={{flexDirection:mobile?'column':'row',flexWrap:mobile?'nowrap':'wrap',gap:12}}>{data.quizzes.slice(0,5).map(q=><QuizCard key={api.idOf(q)} q={q} onOpen={openQuiz}/>)}</View>:<Empty title="No published tests yet" message="Published quizzes will appear here."/>}
