...
import {AppShell,Button,Card,Empty,Field,Header,Loading} from '../../components/UI';
import {api} from '../../services/api';
import {colors} from '../../theme';
export default function StudentNotesScreen(){
 const [notes,setNotes]=useState(null),[show,setShow]=useState(false),[title,setTitle]=useState(''),[content,setContent]=useState('');
 const load=()=>api.notes().then(x=>setNotes(api.listOf(x))).catch(e=>Alert.alert('Notes',e.message)); useEffect(load,[]);
