...
export function Button({ title, onPress, variant = 'primary', disabled = false, style }) {
  ...
  return <Pressable onPress={onPress} disabled={disabled} style={({ pressed }) => [
    s.button,
    { backgroundColor: bg, borderColor: variant === 'primary' || variant === 'danger' || variant === 'success' ? bg : '#BFDBFE', opacity: disabled ? 0.45 : (pressed ? 0.82 : 1) },
    style
...
export function ProgressBar({ value = 0, height = 8 }) {
  ...
  return (
    <View style={[s.progressTrack, { height }]}>
      <View style={[s.progressFill, { width: `${safe}%`, height }]} />
    </View>
  );
}
...
export function StatCard({ icon, label, value, tone = 'primary' }) {
  const toneMap = {
    primary: { bg: colors.blueSoft, fg: colors.primary },
    success: { bg: colors.greenSoft, fg: colors.success },
    warning: { bg: colors.orangeSoft, fg: colors.warning },
  ...
  return (
    <Card style={{ flex: 1, minWidth: 220, marginBottom: 0 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
        <View style={{ width: 42, height: 42, borderRadius: 13, backgroundColor: t.bg, alignItems: 'center', justifyContent: 'center' }}>
          <Text style={{ fontSize: 19 }}>{icon}</Text>
        </View>
        <View style={{ flex: 1 }}>
          <Text style={{ color: colors.muted, fontSize: 12, fontWeight: '800' }}>{label}</Text>
          <Text style={{ color: colors.navy, fontSize: 24, fontWeight: '900', marginTop: 2 }}>{value}</Text>
        </View>
      </View>
    </Card>
  );
...
export const s = StyleSheet.create({
  ...
  content: { paddingHorizontal: 20, paddingTop: 22, paddingBottom: 50, width: '100%', maxWidth: 1250, alignSelf: 'center', minHeight: 'calc(100vh - 74px)' },
  ...
  sectionTitle: { flexDirection: 'row', alignItems: 'flex-end', gap: 12, marginTop: 10, marginBottom: 10 },
  sectionHeading: { fontSize: 20, fontWeight: '900', color: colors.navy },
  sectionSubtitle: { fontSize: 12, color: colors.muted, marginTop: 3 },
  progressTrack: { width: '100%', backgroundColor: '#E2E8F0', borderRadius: 999, overflow: 'hidden' },
  progressFill: { backgroundColor: colors.primary, borderRadius: 999 }
});
